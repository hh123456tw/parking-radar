...
- 使用 `localStorage`，不新增登入與會員資料表。
- 最多 8 個常用目的地、5 筆最近查詢。
- 儲存顯示名稱、地址、行政區與座標，不保存完整 API 回應或空位。
- 使用者可新增、重新命名與刪除常用目的地。
- 首頁點選常用目的地後沿用既有手動查詢 API。
...
- `static/app.js` 顯示完整卡與精簡列表，管理常用／最近目的地。
- `templates/index.html` 加入 PWA 標記與常用目的地區塊。
- `static/style.css` 加入費率列、常用按鈕與安裝提示樣式。
...
5. 收藏常用目的地，重新開啟 PWA 後仍存在。
6. 關閉網路時可以開啟應用殼層，但查詢明確顯示需要網路。
7. 日曆與 Overpass 模擬失敗時，原本即時停車推薦仍正常。
